...
class DatasetService:
    ...
    def _save_upload_bytes(self, filename: str, content: bytes) -> str:
    ...
    async def _save_cover(self, cover: Optional[UploadFile]) -> Optional[str]:
        if cover is None or not cover.filename:
            return None
        content = await cover.read()
        if not content:
            return None
        return self._save_upload_bytes(cover.filename, content)
    ...
    def list_datasets(self) -> List[Dict]:
        rows = self.dataset_dao.list_datasets()
    ...
    def update_dataset(self, dataset_id: int, payload: Dict) -> Optional[Dict]:
        ...
        updated = self.dataset_dao.update_dataset(dataset_id=dataset_id, payload=updates)
        if updated is None:
            return None
        return self._to_payload(updated.to_dict(include_internal=True))

    async def update_cover(self, dataset_id: int, cover: UploadFile) -> Optional[Dict]:
        row = self.dataset_dao.get_dataset_by_id(dataset_id=dataset_id)
        if row is None:
            return None
        ...
        updated = self.dataset_dao.update_dataset(dataset_id=dataset_id, payload={"cover_path": cover_path})
    ...
    def delete_dataset(self, dataset_id: int) -> bool:
        ...
        ok = self.dataset_dao.delete_dataset(dataset_id=dataset_id)
        if ok:
    ...
    async def upload_dataset(
        ...
        upload_list = [x for x in (files or []) if x is not None and x.filename]
