...
from utils.auth_guard import SESSION_COOKIE_KEY, assert_admin_user, get_login_user
...
def logout(request: Request, response: Response):
    ...
    if not session_id:
        session_id = str(get_login_user(request).get("session_id") or "").strip()
    if not session_id:
...
def get_current_session(request: Request, response: Response):
    current_user = get_login_user(request)
    session_id = str(request.cookies.get(SESSION_COOKIE_KEY) or current_user.get("session_id") or "").strip()
    if not session_id:
        raise HTTPException(status_code=401, detail="Not logged in.")
